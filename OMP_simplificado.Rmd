---
title: "CO2AO - Caso práctico extended OMP"
author: "Marcos Fontela, 2019"
output:
  html_document: 
    toc: true
...
list.of.packages <- c("tidyverse", "knitr", "readxl", "ggrepel", "marelac", "plot3D", "DT", "readr", "pracma")
...
data <- read.delim("C:/UsersXXXXXXXXXXXXXXXXXXXXXXXXXX/INPUT_botellas.txt", na.strings="-999") #Actualiza con la ruta de la carpeta donde hayas guardado el archivo
SWT <- read_excel("C:/Users/XXXXXXXXXXXXXXXXXXXXXXXXXX/INPUT_SWT.xlsx")#Actualiza con la ruta de la carpeta donde hayas guardado el archivo
MixingFigures <- read.delim("C:/Users/XXXXXXXXXXXXXXXXXXXXXXXXXX/INPUT_MixingFigures.txt", header=FALSE)#Actualiza con la ruta de la carpeta donde hayas guardado el archivo
...
W<-data.frame(Mass=100, Tpot=20, Sal=10, SiO4=2, 
              NO3=3, NO=3,NO30=3,
              PO4=2, PO=2,PO40=2,
              O2=2, O20=2)
...
Ya tienes cargados los **datos iniciales** (puedes ver la tabla entera haciendo scroll hacia la izquierda):
...
Y también las  propiedades tipo de las masas de agua (SWT) que, *apoyándote en bibliografía* consideras:
...
También cargaste las posibles Mixing Figures:
...
Siempre es bonito ver los datos de botella con las SWT consideradas, ¿no crees? Además te ayuda a ver si escogiste bien las masas de agua tipo.
...
Si vuelves a ver ahora las dos tablas, verás que hay nuevas columnas (cada vez más a la izquierda, sí...):
...
Lo siguiente fue **definir una muestra concreta**, en este ejemplo vamos a caracterizar la composición de masas de agua de la **muestra `r observation`.** La muestra seleccionada tiene esta composición:
...
FigurademezclaUNO<-MixingFigures[1,] #La primera fila
  ...
  #¿Cómo? convirtiéndolos desde lo que son ahora a lo que pide la función.
...
```
¡Tremendo residuo.normalizado! Parece que esta figura de mezcla no es la que mejor le va, ¿no crees? Que te toca ahora... pues volver a plantear este mismo fragmento del script con la siguiente figura de mezcla... o meterlo en un **loop for** que lo haga por ti... ¡ya verás como no ye tan difícil!

![](https://media.giphy.com/media/3ohzAsmzLqsfvbvCs8/giphy.gif)
...
Ya sabes hacerlo para una muestra, así que ahora que ya eres un experto en R, que será mejor, ¿hacerlo muestra a muestra o montar un *for loop* para todas? ;)
